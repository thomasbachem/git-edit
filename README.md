...
A failure pauses the operation with nothing applied and nothing consumed: the built result stays in the worktree, the failing commit is named with an inspect hint (the resume puts the worktree back on the built result if that checkout moved it), `git edit --continue` re-verifies, `git edit --no-verify --continue` applies the result anyway, and `git edit --abort` cancels — for a fold, the staged changes are simply still staged. The plumbing modes (`-M`, pathspec `--split`) reuse existing trees byte-for-byte and run no verification; so does `--exec`, whose command is your own.
