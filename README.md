...
If the autosquash hits a merge conflict (the agent's change overlaps with a later commit that also modifies the same lines), the script doesn't auto-rollback. Instead it pauses, mirroring `git rebase`'s own pause-on-conflict pattern, and prints actionable detail. Every conflict-capable rebase runs with `rerere` enabled — from the step that conflicts through the one that resolves it, so a resolution is recorded even with `rerere.enabled` unset globally. When a cascade re-conflicts the same hunk, or a retry of the whole operation hits it again, that resolution is replayed, staged, and the rebase continued **without pausing**, with a line naming what was replayed. Only a conflict with no recorded resolution pauses — a replayed one is marker-free, which is exactly what tells the two apart (learned resolutions live in the repo-wide `rr-cache`):
