...
By default the command runs at the primary commit the operation authored (a fold's amended commit, an edit's) and at the new tip; `--verify-span` — or a standing `git config edit.verifySpan true`, which names a tier and not a check, so it raises nothing where no command is configured — upgrades to every rebuilt commit, since a green tip proves nothing about the span. A default-tier run that left commits in between unchecked says how many, and names the tier that covers them: a bare "Verified 2 commit(s)" over a span of seven reads as verified when it was sampled. Each run happens in the operation's isolated worktree checked out at that commit — with `edit.worktreeLink` links already in place, so `node_modules` and friends are there — and `GIT_EDIT_VERIFY_COMMIT` names the commit under test. Verification runs against the linked environment — the checkout's current dependencies, not each commit's own — deliberately: every verdict below is a comparison, and holding the environment constant across both sides is what lets a failure be attributed to the operation rather than to dependency drift.
...
**Judge a pause by its trailer first, then by its unmerged index entries — never by grepping for markers.** A marker grep conflates states that need different responses:

- **A `paused – verify failed` trailer** — nothing is unmerged and nothing is to be resolved: the result is built and your own check rejected it. Editing that worktree is refused rather than folded in; apply with `--no-verify --continue` and repair the named commit with its own `git edit` pass, or `--abort`.
- **A `conflict` trailer with no unmerged files at all** — the current step became empty (its changes are already upstream): skip it, nothing to resolve.
